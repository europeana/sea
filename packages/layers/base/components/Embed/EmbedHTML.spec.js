...
const script =
  '<script src="https://www.example.eu/script.js" onload="console.log(`loaded`)" crossorigin="anonymous"></script>';
const inlineScript =
  '<script>console.log("This is an inline test script")</script>';
const fixtures = {
  static: {
    width: 640,
    height: 480,
...
describe("components/Embed/EmbedHTML", () => {
  ...
  describe("responsive embed wrapper", () => {
    it("is used when responsive is `true` and height and width are present", () => {
      ...
      const iframe = wrapper.find(
        '[data-qa="responsive embed wrapper"] [data-qa="html embed"] iframe',
      );
      expect(iframe.exists()).toBe(true);
    });
